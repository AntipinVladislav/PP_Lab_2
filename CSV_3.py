...
def copyRand(randlist):
    classtag = 'Cats'
    for i in range(1000):
        shutil.copyfile(f'dataset/Cats/{str(i).zfill(4)}.jpg', f'dataset/{randlist[i]}.jpg')
    classtag = 'Dogs'
    for i in range(1000):
        shutil.copyfile(f'dataset/Dogs/{str(i).zfill(4)}.jpg', f'dataset/{randlist[i+1000]}.jpg')
